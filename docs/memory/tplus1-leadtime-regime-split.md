...
**Re-litigation guard (2026-06-14):** two post-hoc NO gates were tested against the 182-trade settled book and BOTH REJECTED — (H1) tightening `MAX_EDGE_ABS` 0.40→0.35 (bands non-monotonic; 0.40 sits in profitable territory; the >0.60 losers were YES trades already killed by `ENABLE_YES_BETS=False`), and (H2) a *maximum*-ensemble_std / high-std-narrow-bucket NO gate. **H2's sign is backwards: high std is the PROFITABLE NO regime** (block-NO-if-std≥1.0 removes a +12.9% ROI cohort; ≥1.25 removes +23%) — the std floors go *up*, not down. The real (mild) leak is LOW-edge NO at std extremes, opposite of both. Don't re-run this dead end.
